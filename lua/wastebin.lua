...
M.setup = function(opts)
	...
	vim.cmd([[
	command! -range -nargs=? WasteCopy :lua require("wastebin")._internal_copy_cmd({ args = <q-args>, range = <range> })
	]])
end
...
M._internal_copy_cmd = function(params)
	vim.ui.select({ "yes", "no" }, {
		prompt = "Really paste to " .. M.config.url .. "?"
	},
		function(choice)
